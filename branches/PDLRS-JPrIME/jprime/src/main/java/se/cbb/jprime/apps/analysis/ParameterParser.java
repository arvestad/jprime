...
import se.cbb.jprime.apps.analysis.Parameters;
...
public class ParameterParser {
	 ...
	 */
	public static RBTree getHostTree(Parameters ps, BufferedWriter info) {
		try {
	...
	public static Triple<RBTree, NamesMap, TimesMap> getHostTree(String rawStree)
	{
		try {
			PrIMENewickTree sRaw = PrIMENewickTreeReader.readTree(rawStree, false, true);
			RBTree s = new RBTree(sRaw, "HostTree");
...
//			}
	 ...
	 */
	public static RBTreeArcDiscretiser getDiscretizer( RBTree S, NamesMap names, TimesMap times, int discMin, int discMax, double discTimestep, int discStem) {
		return new RBTreeArcDiscretiser(S, names, times, discMin, discMax, discTimestep, discStem);
	}
...
//	}
	...
	public static LinkedHashMap<String, Integer> getGenePseudogeneMap(Parameters ps) {
		try {
			return GenePseudogeneMapReader.readGenePseudogeneMap(new File(ps.files.get(2)));
		} catch (Exception e) {
